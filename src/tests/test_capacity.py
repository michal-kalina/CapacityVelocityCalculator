...
class TestCapacity:
    ...
    def test_capacity_create_raise_exception(self):
        with pytest.raises(
            TypeError, match=r"Unknown type for '.*?' arg"
        ) as exception_info:
            # Arrange
            # Act
    ...
    def test_people_parametrized(
        self, get_capacity_data, test_date, test_data, expected_capacity_percentage
    ):
    ...
    @pytest.mark.parametrize(
        ["test_date", "test_data", "actual_velocity_sp", "expect_velocity_sp"],
        [
            ("2021-07-19", [], 50, 0),  # 0%
            ("2021-07-19", [{"lp": 1, "days": 14}, {"lp": 2, "days": 14}], 50, 0),  # 0%
            (
                "2021-07-19",
                [{"lp": 1, "days": 9}, {"lp": 2, "days": 10}],
                50,
                12,
            ),  # 25%
            ...
            (
                "2021-07-19",
                [{"lp": 1, "days": 0}, {"lp": 2, "days": 0}],
                50,
                50,
            ),  # 100%
        ],
    )
    def test_capacity_calculate_actual_capacity(
        self,
        get_capacity_data,
        test_date,
        test_data,
        ...
        # Act
        actual_calculated_presence: int = capacity.calculate_actual_capacity(
            actual_velocity_sp
        )
